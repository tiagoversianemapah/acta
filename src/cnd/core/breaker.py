...
def pode_despachar(conn: sqlite3.Connection, orgao: str) -> bool:
    ...
    if atual.aberto_ate and tempo.agora_iso() >= atual.aberto_ate:
        conn.execute(
...
def _faixa_cooldown(p: ParametrosBreaker,
    ...
    if (str(desfecho or "") == str(Desfecho.ERRO_TECNICO)
        ...
        return p.cooldown_erro_inicial_s, (teto if teto is not None else p.cooldown_maximo_s)
    return p.cooldown_inicial_s, p.cooldown_maximo_s
